...
{#each Object.entries(unitData.equippedItems) as [itemName, itemData] (itemName)}
  {@render entry(itemName, itemData, () => builderStore.unequipItem(unitName, itemName), itemData.costForUnit * itemData.count)}
{/each}

{#each Object.entries(unitData.equippedUpgrades) as [upgradeName, upgradeData] (upgradeName)}
  {@render entry(upgradeName, upgradeData, () => builderStore.unequipUpgrade(unitName, upgradeName), upgradeData.cost * upgradeData.count)}
{/each}

{#each Object.entries(unitData.addedStands) as [standName, standData] (standName)}
  {@render entry(standName, standData, () => builderStore.removeStand(unitName, standName), standData.points * standData.count)}
{/each}