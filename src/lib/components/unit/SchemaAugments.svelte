...
{#snippet entry(name: string, data: ISchemaMagicItem | ISchemaUpgrade | ISchemaUnit, onclick: () => void, cost: number)}
  ...
  <div class="flex gap-x-4 select-none cursor-pointer hover:bg-gray-200" onclick={onclick}>
    <div>{name}</div>
    <div>{data.type}</div>
    <div>{cost}</div>
  </div>
...
{#each builderStore.getUnitEquipableItems(unitData) as [itemName, itemData] (itemName)}
  {@render entry(itemName, itemData, () => builderStore.equipItem(unitName, itemName), getUnitItemCost(unitData, itemData))}
{/each}

{#each builderStore.getUnitEquipableUpgrades(unitData) as [upgradeName, upgradeData] (upgradeName)}
  {@render entry(upgradeName, upgradeData, () => builderStore.equipUpgrade(unitName, upgradeName), upgradeData.cost)}
{/each}

{#each builderStore.getAttachableStands(unitData) as [standName, standData] (standName)}
  {@render entry(standName, standData, () => builderStore.addStand(unitName, standName), standData.points)}
{/each}